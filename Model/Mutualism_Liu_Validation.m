...
prob_disturb = 0.1;                 % the probability of wind disturbance in animal movements
...
prob_pollen = 1;                    % the probability of the pollen production rate

%% Initial Conditions
init_size = 80;         % initial landscape size n x n
...
envir_length = 7;       % num of parameters of the environment
...
reproduction_rate = @(num_animal, num_plant) ...
                    num_plant*0.0001/num_animal;
...
plants = zeros(init_num_plant*50,plant_length);
...
for days = 2:timestep:totalsteps
    ...
    for plant = 1:helper(plant_next)
        ...
        if plant_maturity_date < plants(plant, plant_age) && plants(plant, plant_age) < plant_maturity_date + seed_period
            if plants(plant, pollinated) == 1 && rand < prob_landing
                ...
                plants(helper(plant_next), plant_age) = -60;
                output = wind_impact(environment(days, wind_angle), ...
    ...
    for animal = 1: helper(animal_next) - 1
        ...
        % movement with enough pollen in the environment
            ...
            animals(animal, animal_x) = C(centroid,1) + (-2*sqrt(init_size)+rand*sqrt(init_size)*2);
            animals(animal, animal_y) = C(centroid,2) + (-2*sqrt(init_size)+rand*sqrt(init_size)*2);
        ...
        if animals(animal, animal_sex) == 1 && rand < reproduction_rate(environment(days, num_animal), environment(days, num_plant))
...
legend(h, 'dead plant cluster', 'alive plant cluster', 'seed', 'alive animal', 'egg', 'pollen')
